...
export function PhotoModalContent({
  ...
  // Sync ref with latest navigation state (safe - refs don't trigger re-renders)
  ...
  useEffect(() => {
    ...
    const drawerEl = contentRef.current?.closest("[data-vaul-drawer]") as HTMLElement | null
    if (!drawerEl) return
        ...
        <AnimatePresence initial={false} custom={direction}>
          ...
          >
            <Image
              src={photo.imageUrl}
              alt={
                photo.dish_name || photo.description_en || photo.description_cn || "A homemade meal"
              }
              fill
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="pt-3 text-right">
            <span className="font-accent text-lg text-ink-tertiary">With love ♡</span>
          </div>
        </motion.div>
      </AnimatePresence>
    </div>
